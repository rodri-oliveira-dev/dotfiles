...
## Manual releases and immutable tags

The `Publish release` workflow (`.github/workflows/release.yml`) is triggered **manually only**. After its pull request is merged, open **Actions → Publish release → Run workflow**, select `main`, and enter a stable SemVer version **without** the `v` prefix, for example `1.0.0`. The workflow creates a new lightweight tag `v1.0.0` at the current `main` commit and publishes a GitHub Release with automatically generated notes. It does not build or upload binaries.

The workflow rejects non-`main` runs, malformed versions (including prerelease/build metadata), stale selected commits, and previously created tag names during normal publication. An explicit recovery option is available for an existing tag that belongs to the current `main` history and does not yet have a release. Publication runs are serialized and are not automatically canceled. Only the publishing job receives `contents: write`; no external action, PAT, or repository secret is needed. **Do not execute the first release until the tag ruleset below is active.**
...
If a tag exists but release publication fails, **do not delete or move it**. After merging the fix into `main`, run **Actions → Publish release → Run workflow**, select `main`, enter the existing version (for example, `1.0.0`), and enable **recover_existing_tag**. Recovery checks that the existing lightweight tag references a commit in the current `main` history and that no release for that tag is present, then publishes the release without changing the tag. A normal run with an existing tag still fails. If the tag points outside the `main` history, do not attempt to force it through recovery. The tag ruleset protects Git tags; it does not, by itself, make release descriptions or assets immutable.
