...
Se uma tag já existir, mas a publicação da release falhar, **não exclua nem mova a tag**. Após integrar a correção na `main`, acesse **Actions → Publish release → Run workflow**, selecione `main`, informe a versão existente (por exemplo, `1.0.0`) e habilite **recover_existing_tag**. A recuperação verifica se a tag leve existente aponta para um commit do histórico atual da `main` e se ainda não existe uma release para ela; então publica a release sem alterar a tag. Uma execução normal com uma tag existente continua falhando. Se a tag apontar para fora do histórico da `main`, não tente forçar a recuperação. O ruleset protege as tags Git, mas não torna, por si só, as descrições e os artefatos da release imutáveis.
